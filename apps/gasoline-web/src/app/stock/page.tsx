				...
				<section className="bg-white p-4 rounded-xl border border-gray-150 shadow-sm flex flex-col gap-4">
					<div>
						<h2 className="text-sm font-bold text-gray-900 mb-1 uppercase tracking-wide flex items-center gap-1.5">
							<Sliders className="w-4 h-4 text-orange-500" /> Penyesuaian Stok Langsung (CRUD)
						</h2>
						<p className="text-xs text-gray-500">
							Opsi darurat untuk langsung mengubah angka fisik stok jerigen dan botol secara instan
							tanpa alur kalkulasi shift.
						</p>
					</div>
					...
					{adjSuccess && (
						<div className="bg-green-50 text-green-700 text-xs p-2.5 rounded-lg font-bold border border-green-100 flex items-center gap-1.5">
							<Check className="w-4 h-4" /> Stok berhasil disesuaikan secara langsung!
						</div>
					)}

					<form onSubmit={handleSaveAdjustments} className="flex flex-col gap-4">
						<div className="flex flex-col gap-1.5 border-b border-gray-100 pb-3">
							<label htmlFor="adj-jerigen" className="text-xs font-bold text-gray-700">
						...
						<div className="flex flex-col gap-3">
							...
							{products.map(p => (
								<div key={p.id} className="flex justify-between items-center gap-4">
									<label htmlFor={`adj-${p.id}`} className="text-xs font-semibold text-gray-700">
										{p.name} ({p.volume}L)
									</label>
									<div className="w-24">
										<input
											...
											onChange={e => setAdjBottles({ ...adjBottles, [p.id]: e.target.value })}
				...
				<div className="flex flex-col gap-5">
					...
					<section className="bg-white p-4 rounded-xl border border-gray-150 shadow-sm flex flex-col gap-4">
						<div>
							...
							<p className="text-xs text-gray-500">
								Kelola tipe botol bensin eceran yang dijual beserta harga beli dan harga jualnya.
							</p>
						</div>
								...
								>
									<div className="flex flex-col gap-0.5">
										...
										<span className="text-[10px] text-gray-400 font-semibold">
											Volume: {p.volume}L | Beli: {formatPrice(p.costPrice)} | Jual:{' '}
