...
export default function FinancePage() {
	const { dailyRecaps, salaryPayments, fetchRecapsFromCloud, fetchSalaryFromCloud } =
		useGasolineStore()
				...
				<div className="grid grid-cols-2 gap-4">
					<div className="flex items-start gap-2.5">
						...
						<div className="flex flex-col">
							...
							<span className="text-sm font-bold text-gray-800">{formatCurrency(totalCashIn)}</span>
						</div>
					</div>
						...
						{dailyRecaps.map(recap => (
							...
							>
								<div className="flex items-center justify-between border-b border-gray-50 pb-2">
									<span className="text-xs font-black text-gray-800">📅 {recap.date}</span>
									<span
										className={`text-xs font-bold ${
											recap.cashSummary.netFinanceFlow >= 0 ? 'text-green-600' : 'text-red-600'
										}`}
									>
