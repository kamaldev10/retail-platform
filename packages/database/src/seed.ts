...
async function main() {
	...
	if (!name || !email || !password) {
		console.error(
			'⚠️ Error: SEED_ADMIN_NAME, SEED_ADMIN_EMAIL, and SEED_ADMIN_PASSWORD must be set in your .env file.',
		)
		process.exit(1)
	}
	...
	if (supabaseUrl && serviceKey) {
		...
		console.warn(
			'⚠️ Warning: NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY is missing. Skipping Supabase Auth creation.',
		)
	}

	// 2. Upsert admin user inside users table in PostgreSQL using Raw SQL repository
	console.log('🔄 Upserting admin record in users table via Raw SQL...')
